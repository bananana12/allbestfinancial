...
const PHONE_REPEAT_WINDOW_MS = 10 * 60 * 1000;
...
function validatePayload(payload) {
  ...
  if (clean.company) {
    return { ok: false, error: 'Submission rejected.', clean };
  }
  if (!clean.name || !clean.phone) {
    return { ok: false, error: 'Name and phone are required.', clean };
  }
  if (/[0-9０-９]/.test(clean.name)) {
    return { ok: false, error: 'Name cannot contain numbers.', clean };
  }
...
function pruneMap(map, now) {
  for (const [key, value] of map.entries()) {
    ...
    if (!latest || now - latest > RATE_LIMIT_WINDOW_MS) map.delete(key);
  }
}

function checkRateLimit(req, payload) {
  const now = Date.now();
  pruneMap(ipBuckets, now);
  pruneMap(recentPhones, now);

  const ip = getClientIp(req);
  const events = (ipBuckets.get(ip) || []).filter(ts => now - ts < RATE_LIMIT_WINDOW_MS);
  if (events.length >= RATE_LIMIT_MAX) {
    return { ok: false, status: 429, error: 'Too many submissions. Please try again later.' };
  }

  const phone = normalizePhone(payload.phone);
  const lastPhoneSubmit = phone ? recentPhones.get(phone) : 0;
  if (lastPhoneSubmit && now - lastPhoneSubmit < PHONE_REPEAT_WINDOW_MS) {
    return { ok: false, status: 429, error: 'This phone number was submitted recently. Please try again later.' };
  }

  events.push(now);
  ipBuckets.set(ip, events);
  if (phone) recentPhones.set(phone, now);
  return { ok: true };
}
...
async function handleApply(req, res) {
  try {
    ...
    const limited = checkRateLimit(req, validated.clean);
    if (!limited.ok) {
      sendJson(res, limited.status, { ok: false, error: limited.error });
      return;
    }
